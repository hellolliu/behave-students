...
public class AdminAdminController {
    ...
    public Object list(String username,
        ...
        List<BehaveAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(BehaveAdmin admin) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody BehaveAdmin admin) {
        ...
        List<BehaveAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    public Object read(@NotNull Integer id) {
        BehaveAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }
    ...
    @PostMapping("/update")
    public Object update(@RequestBody BehaveAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody BehaveAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        BehaveAdmin currentAdmin = (BehaveAdmin) currentUser.getPrincipal();
